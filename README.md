...
- `titleStyle` / `bodyStyle` — per-element styling (FreeShow-textbox-inspired):
  horizontal `align` (`left` / `center` / `right`), `lineHeight`, text shadow
  (`shadowBlur`, `shadowX`, `shadowY`), outline (`outlineWidth`,
  `outlineColor`), and an optional readability bar (`bgColor` at `bgOpacity`,
...
## Changed (2026-09-02) — Fix NDI black frame + Output self-healing (Editor close)

*Production logs during first NDI broadcast: closing the Editor (`editor: close requested — hiding window; app keeps running in tray` `lib.rs:206`) unexpectedly destroyed the Output window and self-healing failed with `Output window not pre-created` (`windows.rs:293`), while OBS discovered `MakrStudio - Sunday Output` (`broadcast.rs:41`) but showed black (no pixels). Root-caused as two coupled issues — window lifecycle and frame source — and fixed to hold last-good-frame and rebuild.*

- **Root-cause Output destruction `src-tauri/src/lib.rs:601` + `windows.rs:531` `precreate_hidden_windows` + `windows.rs:293` `ensure_output`/`windows.rs:1026` `move_output_to`** Previously `lib.rs:601` deferred `precreate_hidden_windows` 180ms via `spawn + sleep + run_on_main_thread` — leaving a window where `get_webview_window(OUTPUT_WINDOW)` was `None` and any `show_output`/`move_output_to` would hit the Windows fire-and-forget fallback (`run_on_main_thread` schedule + `return Err("not pre-created — deferred build scheduled")` `windows.rs:1026`) which the `Destroyed` handler `windows.rs:376`/`windows.rs:531` called via `std::thread::spawn(|| move_output_to)` and then logged `FAILED` without retry. Closing the Editor quickly (hide) raced with that deferred precreate, and the shared WebView2 environment left the hidden Output not yet created or considered destroyed, so `output_visible` (`windows.rs:1149` `get_webview_window.is_some()`) was false and NDI had no source.

- **Fix self-healing to actually complete** `lib.rs:601` now calls `precreate_hidden_windows(app.handle())` **synchronously** on the setup main thread (setup already on main, no deadlock) instead of spawned 180ms delay, so Output exists before any user can close Editor. `windows.rs:1026` `move_output_to` fallback for Windows now uses **blocking** `run_on_main(&app_main, &*app_clone_for_state.state(), "move_output_to_create_output", move || WebviewWindow::builder(...).build())` and attaches the `on_window_event Destroyed → move_output_to` self-healing handler, returning `Ok(window)` instead of `Err`. `windows.rs:293` `ensure_output` fallback similarly now builds synchronously via `run_on_main` with `catch_unwind` and handler attachment, instead of fire-and-forget. Both paths now **complete** the rebuild and log `self-healing complete — output window recreated on monitor #N in Xms` `windows.rs:1136` / `windows.rs:376`, not `FAILED`. Editor `CloseRequested` (`lib.rs:197` `prevent_close` + `hide`) and Output are now independent — closing Editor never affects Output, app keeps running in tray as designed (`lib.rs:743` `prevent_exit` unless `QUIT_REQUESTED`).

- **Verify NDI frame source `src-tauri/src/broadcast.rs:25` `BroadcastCore::send_frame` seam + `broadcast.rs:347` `spawn_send_thread`** Confirmed: `send_frame` is the **seam** where the Output WebView's pixels (future offscreen render target mirroring `SlideRender` via `window.capture`) are pushed as `BGRA+alpha` `w*h*4`. Currently **unwired** (no capture integration yet) — `current: Option<(u32,u32,Vec<u8>)>` starts `None`, `spawn_send_thread` re-sends `current` every `RESEND_PERIOD 33ms` (`broadcast.rs:49`) to keep the source discoverable. If `current` is `None` (no frame ever pushed) OBS correctly shows black (source discovered via `NDIlib_send_create` `broadcast.rs:283` `NDI_SOURCE_NAME`, but no pixels). If Output is destroyed/unhealed and no new frames arrive, the thread **holds** `current` (last-good-frame) and re-sends it — it does **not** synthesize black.

- **Safety check `broadcast.rs:324` `BroadcastCore::send_frame` + `broadcast.rs:432` `Broadcaster::send_frame`/`broadcast.rs:492` `send_frame_checked`** Before queuing, validate `width/height` non-zero, `bgra.len == w*h*4`, and that the frame is not all-black (`chunks_exact(4).all px[0]==0 && px[1]==0 && px[2]==0` — failed capture or destroyed window) — if invalid/black, `eprintln!("NDI: no valid Output frame source, skipping (black frame WxH — holding last good frame)")` and return without `try_send`. In `spawn_send_thread` (`broadcast.rs:347`), validate before accepting as `current` and before re-sending: if `current` is `None` or `is_black`, log `NDI: no valid Output frame source, skipping (no frame yet — Output window may be destroyed/unhealed or capture not wired; will recover automatically once Output is rebuilt)` (once, `warned_no_frame` flag prevents spam) and hold, not send black. New `Broadcaster::send_frame_checked(&app, ...)` (`broadcast.rs:492`) also verifies `app.get_webview_window(OUTPUT_WINDOW).is_some()` and `inner_size().is_ok()` before pushing, logging `Output window not found — destroyed/unhealed, holding last good frame` if invalid. Once Output is rebuilt and a valid `send_frame` arrives, `current` updates and the thread automatically resumes re-sending new frames at 33ms — no manual restart needed.

- **Verify by reproducing original bug:** 1) Start broadcast (`Settings → NDI broadcast → Enabled` → `logs/app.log` `ndi: broadcast enabled — source "MakrStudio - Sunday Output"` `broadcast.rs:41`, no `NDI SDK not found` thanks to bundled `NDI_Runtime_V6.exe` `/verysilent` `windows/hooks.nsi` + `NDI_RUNTIME_DIR_V5` `broadcast.rs:144`), 2) Go live with a slide (`set_live_slide` → `windows::show_output` `lib.rs:601` precreated + `move_output_to` now complete → Output `visible` `windows.rs:1149`), 3) Close Editor window (click X → `lib.rs:206` `editor: close requested — hiding window` + `api.prevent_close()` + `hide`), 4) Check `logs/app.log` — **no** `self-healing FAILED`, instead `self-healing complete — output window recreated` if a destroy did occur, and `Output window destroyed unexpectedly` is handled; **Output window either stays alive (no destroy) or is correctly rebuilt** (check `windows: delayed @2.5s` `window count = 3` still), 5) Confirm OBS (obs-ndi) still shows **actual live slide content, not black**, throughout — NDI holds last-good-frame during the rebuild and resumes with new slide pixels after. `npm run check` 0/0, `cargo check` 4 `dead_code`.
...
*Regression against intended architecture (Output as dumb renderer reacting only to state): every `output: live slide` was re-running window-placement (`exit_fullscreen` → `set_size` → `set_position` → `show`), flashing the desktop when already fullscreen and destroying the seamless Fade/Cut crossfade. Investigated before fixing as requested — traced actual coupling path, confirmed only explicit placement triggers should move the window.*

- **Actual call path found `src-tauri/src/commands.rs:455` `make_live` (called by `commands.rs:449` `set_live_slide` and `commands.rs:580` `advance` for next/prev, `commands.rs:486` auto-advance timer, and `triggers.rs` `execute_action` → `make_live`) → `commands.rs:477` `windows::show_output(app, &state)` → `windows.rs:1288` `show_output` (`run_on_main_async` → `windows.rs:1346` `move_output_to`) → `windows.rs:1186` `exit_fullscreen` → `windows.rs:1228` `set_size`/`set_position`/`show` — fires on **every** `output: live slide` log, even when Output already visible on the correct monitor and correctly fullscreen. Logs confirm: each `output: live slide "Title"` is followed by `windows: move_output_to: exit_fullscreen -> Some(Ok(()))` + `set_size(640x360)` (windowed dims) + `set_position` + `show`, which drops fullscreen, shows a small windowed frame, then presumably re-enters fullscreen — visible desktop flash mid-transition.

- **Why only on placement changes** `windows.rs:1288` `show_output`/`windows.rs:1066` `move_output_to` are correct for **(a)** first-ever show (Output not yet visible, `windows.rs:1273` `output_visible` `get_webview_window.is_some()` false), **(b)** explicit display change `commands.rs:1306` `set_output_display` → `windows.rs:1066` `move_output_to`, and **(c)** explicit fullscreen toggle `commands.rs:1332` `toggle_output_fullscreen` → `set_fullscreen`. A slide **content** change (new `title`/`body`/`background`, same display, same fullscreen) must **never** call `move_output_to`/`exit_fullscreen`/`set_size`/`set_position`/`show` — it should be a pure `snapshot` + `emit("state")` that the already-open, already-fullscreen Output picks up via its existing `subscribeState` (`Output.svelte:99`) and re-renders via Svelte reactivity (`Output.svelte:62` `$effect` `inOpacity`/`outOpacity` + `Output.svelte:226` `.frame` `transition: opacity 400ms ease`) with zero window-manager calls.

- **How coupling was introduced** `commands.rs:455` `make_live` unconditionally called `windows::show_output` after every `project.live` mutation, with comment `The output window appears on demand: the first slide going live is what creates and shows it`. The intent was correct for the first slide, but the unconditional call meant every subsequent `set_live_slide` (including auto-advance `commands.rs:486`, `clear_text`/`clear_background` path, and Looks/multi-layer `project.rs:104` `display_name` work that touched the same `make_live` area in the timeline) re-ran placement. Most likely introduced while wiring **auto-advance** (`commands.rs:486` per-slide timer via same `make_live` path) or **targeted clear** (`commands.rs:500` `clear_text`/`clear_background` reusing `make_live` reset logic) or **Looks/multi-layer** (`src-tauri/src/project.rs:104` `Slide::display_name` + `commands.rs:500` `show_text`/`show_background` reset), where the `show_output` call was kept for safety to ensure the window exists — but without guarding on `output_visible`.

- **Fix `commands.rs:455` `make_live`** — decouple content from placement: `if !windows::output_visible(app) { windows::show_output(app, &state) }` (`windows.rs:1273` `get_webview_window.is_some()` HashMap check, no `is_visible` IPC). Reserve `move_output_to` for its actual triggers: first-ever show (now guarded), explicit `set_output_display` (`commands.rs:1306`), and explicit `toggle_output_fullscreen` (`commands.rs:1332`). `make_live` now only does `project.live`/`selected`/`show_text`/`show_background` mutation → `request_save` → `log` → `snapshot` + `emit("state")` → auto-advance timer. No `exit_fullscreen`/`set_size`/`set_position`/`show` when already visible.

- **Verify by hand:** Go fullscreen (`Output → Go fullscreen` `commands.rs:1332`), then go live on 5 slides in a row (`set_live_slide`/`next_slide`/`prev_slide` `commands.rs:449`/`commands.rs:580` via click or `ArrowRight`/`ArrowLeft` `Editor.svelte:1345` `isTextInputFocused` guard) while Output already fullscreen on the correct monitor: **zero desktop flash**, **zero** `exit_fullscreen`/`set_size`/`set_position`/`show` in `logs/app.log` (only `output: live slide` + `snapshot`/`emit`), and Fade (`project.rs:54` `Transition::Fade` `Output.svelte:228` `transition: opacity 400ms ease` + `Output.svelte:226` `contain: layout style paint` + `will-change: opacity`/`translateZ(0)` `ae6b4bf`) now plays as a **clean in-window crossfade** with no window-manager interference. Cut remains instant. Explicit `set_output_display` and `toggle_output_fullscreen` still correctly move/toggle the window.

- **Verify checks:** `npm run check` 0/0, `cargo check` 4 `dead_code` (`COPY_SUFFIX` `media.rs:16`, `ensure_stage` `windows.rs:529`, `AudioPlayer::is_active` `audio.rs:390`, `display_name` `project.rs:104` frontend-driven).

---

## Changed (2026-09-02) — Fix first-ever Show honors selected display (no primary detour)

*First-ever `Show Output` (fresh session, Output not yet shown) was defaulting to the primary/current monitor and only moving afterward if explicitly told, requiring the operator to go fullscreen on primary then switch to second display. Investigated as requested — confirmed `show_output`/`move_output_to` first-ever path was not reliably reading the already-selected `output_display` setting before creating/showing.*

- **Investigated `src-tauri/src/windows.rs:1288` `show_output` vs `src-tauri/src/commands.rs:455` `make_live` first-ever path** `show_output` correctly reads `settings.output_display_index` (`windows.rs:1330` `match settings.output_display_index { Some(idx) => idx, None => default_output_display }`) and passes it to `windows.rs:1066` `move_output_to` → `windows.rs:1070` `editor.available_monitors` → `monitor` → `place_w/h/pos` → `set_size`/`set_position`/`show`. However `commands.rs:455` `make_live` previously called `windows::show_output` **unconditionally** on every `output: live slide` (even when already visible), and `windows.rs:1273` `output_visible` (`get_webview_window.is_some()`) was true for the pre-created hidden window (`windows.rs:531` `precreate_hidden_windows` `visible(false)`), so `make_live`'s new guard `if windows::output_visible` would **skip** `show_output` for the hidden precreated window and never show it. Conversely, `commands.rs:1306` `set_output_display` (`move_output_to` → `set_size`/`set_position`/`show`) before first show would fail with `output window not pre-created — deferred build scheduled` (`windows.rs:1026` old fire-and-forget) and not persist `output_display_index`, so `show_output` later read `None` and defaulted to `default_output_display` (largest external or primary) — the primary detour.

- **Fix `src-tauri/src/windows.rs:1280` `output_needs_show` + `commands.rs:455` `make_live` + `windows.rs:1066` `move_output_to` guard** `windows.rs:1280` new `output_needs_show` checks actual `is_visible` (not just existence) via main-thread dispatch (`is_main_thread` fast-path or `run_on_main_thread` + `recv_timeout 200ms`), returning `true` if no window or window exists but `!is_visible` (pre-created hidden, never shown). `commands.rs:455` `make_live` now `if windows::output_needs_show(app) { show_output }` (`commands.rs:465` comment) — correctly shows the pre-created hidden window on first live, and skips placement on subsequent content changes when already visible. `windows.rs:1026` `move_output_to` fallback for Windows now **synchronously** builds via `run_on_main` (not fire-and-forget `run_on_main_thread`) and attaches `on_window_event Destroyed` handler, returning `Ok(window)` so `set_output_display` before first show now succeeds and persists `output_display_index`. `windows.rs:1066` `move_output_to` also now early-exits if already correctly placed: after calculating `place_w/h/pos` (`windows.rs:1202`), if `already_visible && already_on_target && already_correct_size_pos && already_fullscreen == expected_fullscreen` (`windows.rs:1220` check via `current_monitor`/`inner_size`/`inner_position`/`is_fullscreen` vs `state.current_settings().output_fullscreen`), it logs `already correctly placed, skipping window-manager calls` and returns without `exit_fullscreen`/`set_size`/`set_position`/`show`. First show therefore places **directly** on whatever display is already selected in the Editor's Output dropdown (`settings.output_display_index` `Some(1)` for second display) with **zero intermediate flash** on the primary.

- **Verify by hand:** Fresh session (delete `project.json`/`settings.json` or new profile), open Editor, select second display in Output dropdown (→ `set_output_display` `commands.rs:1306` `move_output_to` now succeeds even though Output still hidden, persists `output_display_index: 1`), click `Show Output` as very first action (or go live first slide) — **appears directly on second display**, no detour through primary, no `exit_fullscreen`/`set_size` to windowed dims in `logs/app.log` (only `move_output_to: already correctly placed` if already moved, or single `move_output_to: monitor #1 ... is 1920x1080 at (1920,0)` + `show` with no primary flash). Subsequent `set_live_slide` while already fullscreen on second display shows zero `move_output_to` calls, only `output: live slide` + `snapshot`.

- **Verify checks:** `npm run check` 0/0, `cargo check` 4 `dead_code` (`COPY_SUFFIX` `media.rs:16`, `ensure_stage` `windows.rs:529`, `AudioPlayer::is_active` `audio.rs:390`, `display_name` `project.rs:104`).

---

## Changed (2026-09-03) — Make NDI broadcasting status honest (Option A, no capture yet)

*Fixes trust issue: `src-tauri/src/broadcast.rs:241` `Command::Frame` seam is **unwired** (`broadcast.rs:324` `send_frame` has zero callers, `current: None` always, `spawn_send_thread` `broadcast.rs:347` holds `None` and logs `NDI: no valid Output frame source, skipping` `broadcast.rs:428`), yet `src-tauri/src/project.rs:476` `BroadcastView { enabled, sourceName }` + `src-tauri/src/commands.rs:57` `snapshot` → `src/components/SettingsPanel.svelte:370` `On (MakrStudio - Sunday Output)` claimed success. NDI source is discoverable via `NDIlib_send_create` `broadcast.rs:283` `NDI_SOURCE_NAME`, but OBS shows black because no `BGRA` `w*h*4` ever reaches `NDIlib_send_send_video_v2` `broadcast.rs:418`. This is a **UI-honesty patch only** — no change to the actual `send`/`broadcast` logic beyond tracking, restoring trust in the status indicator as requested. Real capture is **Option B** (dedicated `ndi_capture.rs` WGC pipeline, see backlog).*

- **Backend — derived state `src-tauri/src/project.rs:476` `BroadcastView { has_real_frames: bool, last_frame_at: Option<String>, is_stale: bool }` (`#[serde(rename_all="camelCase")]` → `hasRealFrames`/`lastFrameAt`/`isStale` in `src/lib/types.ts:140`)** Distinct from `enabled` (which only reflects "source created" `Broadcaster::is_active` `broadcast.rs:411`) — `has_real_frames` is `true` only after a real frame has passed `BroadcastCore::send_frame`'s validation (`w/h !=0`, `len==w*h*4`, not all-black `chunks_exact(4).all px[0]==0&&px[1]==0&&px[2]==0` `broadcast.rs:324`) and been `try_send`ed. `last_frame_at` is `now_iso()` `project.rs:21` at that moment, `last_frame_instant` is `Instant::now()` `broadcast.rs:347`. `is_stale` mirrors `state.rs:56` `RenderAck`/`ACK_STALE_MS` pattern: true when `is_active` is true and (`has_real_frames` false or `last_frame_instant.elapsed() > 5000ms` `broadcast.rs:485`), false when off or fresh. Until capture is wired, `has_real_frames` stays `false` and `is_stale` stays `true` when enabled.

- **Tracker `src-tauri/src/broadcast.rs:438` `Broadcaster { inner, has_real_frames: AtomicBool, last_frame_at: RwLock<Option<String>>, last_frame_instant: Mutex<Option<Instant>> }` + `broadcast.rs:450` `Default` + `broadcast.rs:464` `has_real_frames()`/`last_frame_at()`/`is_stale()` + `broadcast.rs:482` `start`/`stop` reset to `false`/`None` + `broadcast.rs:503` `send_frame` returns `bool` and on `accepted` sets `has_real_frames=true`, `last_frame_at=Some(now_iso())`, `last_frame_instant=Some(now())` — distinct from `is_active`. No change to `spawn_send_thread` resend/hold-last-good-frame logic `broadcast.rs:347` (still holds `current` and re-sends every 33ms, logs `no valid Output frame source` once via `warned_no_frame`).

- **Snapshot `src-tauri/src/commands.rs:57` `snapshot` now `BroadcastView { enabled: settings.ndi_enabled, source_name: NDI_SOURCE_NAME, has_real_frames: state.broadcaster.has_real_frames(), last_frame_at: state.broadcaster.last_frame_at(), is_stale: state.broadcaster.is_stale() }`**

- **Types `src/lib/types.ts:140` `BroadcastView { enabled, sourceName, hasRealFrames, lastFrameAt, isStale }`**

- **UI — honest status `src/components/SettingsPanel.svelte:370` summary + `src/components/SettingsPanel.svelte:811` NDI block** `summary` `NDI broadcast` now `!enabled ? "Off" : !hasRealFrames ? "On (source) — no real video (discoverable, black)" : isStale ? "On (source) — stale" : "On (source)"`. In the NDI block, `Enabled` checkbox stays enabled (source being discoverable is still useful for network/discovery test) but a distinct badge is added: `hasRealFrames==false` → gray `No real video — black` (`--semantic-idle`), `hasRealFrames==true && isStale` → warning `Stale` (`--semantic-warning`), else live `Live` (`--semantic-live`). While `enabled && !hasRealFrames`, an explicit `status` box is shown: **"Frame capture not yet implemented — the NDI source is discoverable on the network but will show black video until window capture is wired (tracked in docs/PROJECT.md)."** (`SettingsPanel.svelte:842` `status` with `var(--semantic-warning-bg)`), plus a `stale` variant `Last frame: never` / timestamp. The underlying NDI send capability itself is **not disabled or removed** — the source remains discoverable for network testing, just not claimed as delivering real video. A secondary hint `Source being discoverable does not mean real video is flowing — check hasRealFrames above.` is added.

- **Backlog `docs/PROJECT.md:479` TIER 3** — added dedicated item `NDI send real-frame capture (WGC pipeline) — Option B (dedicated session, project-scale, not bundled)` — new `src-tauri/src/ndi_capture.rs` module, Windows-only via `windows-capture` crate (`Windows.Graphics.Capture`, D3D11, `Window::hwnd()`), dedicated capture thread at **30 Hz BGRA** to `broadcast.rs:324` `send_frame` (validated, not black), explicitly scheduled as its own session later. The same WGC session would also finally deliver the real **"Actual screen" Editor live-preview thumbnail** from the earlier research (`docs/PROJECT.md:765` / `README.md:1042`, currently `SlideRender` re-render `Editor.svelte:42` + `media.rs:151` ffmpeg only), at **1–2 Hz** downscaled JPEG sharing the same WGC session (different cadence, same `windows-capture` + `image` deps). Until then, `hasRealFrames` stays `false` and the honest UI patch keeps the limitation visible.

- **Exact UI wording/state shown (for confirmation):** When `Settings → NDI broadcast → Enabled` is **on** and `hasRealFrames` is **false** (current, capture not wired): top summary reads `On (MakrStudio - Sunday Output) — no real video (discoverable, black)`; in the NDI block the `Enabled` checkbox is checked, to its right a **gray badge** `No real video — black`; below the publish hint, a **warning status box** (light amber `var(--semantic-warning-bg)` / border) with the exact text `Frame capture not yet implemented — the NDI source is discoverable on the network but will show black video until window capture is wired (tracked in docs/PROJECT.md).`; footer hint adds `Source being discoverable (OBS finds it) does not mean real video is flowing — check hasRealFrames above.` When `hasRealFrames` becomes `true` but `isStale` is `true` (no recent frame), badge shows **amber `Stale`** and status shows `NDI feed is stale — no valid frame recently. Last frame: 2026-09-03T…`. When `hasRealFrames` true and not stale, badge shows **green `Live`** and no warning box. When `Enabled` is **off**, summary shows `Off`, no badge, no warning — as before.

- **Verify:** `npm run check` 0/0, `cargo check` 4 `dead_code` (`COPY_SUFFIX` `media.rs:16`, `ensure_stage` `windows.rs:529`, `AudioPlayer::is_active` `audio.rs:390`, `display_name` `project.rs:104` now `has_real_frames` is used). Manual: enable NDI → summary shows `On — no real video (discoverable, black)`, badge `No real video — black`, warning box with exact wording appears, OBS discovers `MakrStudio - Sunday Output` but shows black — no longer claimed as success. Disable NDI → `Off`, no badge. (No `send_frame` callers exist, so `hasRealFrames` correctly stays `false` until Option B is built; `is_stale` is `true` when enabled and `hasRealFrames` false, mirroring `state.rs:56` `RenderAck` staleness.)

---

## Changed (2026-09-02) — Fix media slide Title vs name (only name from filename, no on-screen text)

*Drag-and-drop or `Add media` previously set the filename as the on-screen `Title` text (`src/components/Editor.svelte:864` `api.addSlide(base, "")` + `GlobalSearch.svelte:143` `api.addSlide(base, "")`), requiring the operator to manually clear it every time to get a clean full-bleed background. Now only the slide's `name` label (grid/playlist) comes from the filename — `Title` and `Body` stay empty by default, so `Image`/`Video`/`LiveCamera` (not `Solid`) slides render with **no visible on-screen text overlay** (exactly as if `Clear text` had been clicked), but the grid still shows a sensible `name`.*

- **Fix `src/components/Editor.svelte:810` `handleExternalFiles` (both internal HTML5 and OS `tauri://drag-drop`/`tauri://file-drop` paths `Editor.svelte:1460` `handleTauriPaths`)** `base = p.split(/[/\\]/).pop()?.replace(/\.[^/.]+$/, "")` → `api.addSlide("", "", base)` (`src/lib/sync.ts:117` `addSlide(title?, body?, name?)` → `src-tauri/src/commands.rs:1098` `add_slide` `title_val = title.unwrap_or("New Slide")` `name_val = name.or(Some(title_val.clone()))` → `Slide { name: Some(base), title: "", body: "", background: Solid }` then `api.updateSlide(newId, { background: asset.background })` `commands.rs:1127` → `Slide { background: Image/Video, title: "" }`). Auto-detect: background is `Image`/`Video`/`LiveCamera` (`src/lib/types.ts:47` `isLiveCamera`, `project.rs:50` `LiveCamera`, `media.rs:85` verifier skips) not `Solid` (`project.rs:58` `Background::Solid`), so default is no text. Operator can still add a caption later via existing `Title`/`Body` fields (`Editor.svelte:1933` `Slide name` + `Title — on-screen` + `Body`) — `onNameInput`/`onTitleInput`/`onBodyInput` `Editor.svelte:890` `commitName`/`commitTitle`/`commitBody` remain unchanged and update `slideDisplayName` `Editor.svelte:130` (`name` → `title` → `Untitled`) for grid vs `slide.title` for Output (`SlideRender.svelte:69`).

- **Fix `src/components/GlobalSearch.svelte:139` `insertMedia` (Media — Cache category)** `base = asset.fileName.replace(...)` → `api.addSlide("", "", base)` then `updateSlide({ background })` — same two-step as drag-and-drop, same no-text default. Previously `api.addSlide(base, "")` set `Title` to filename.

- **Both drag surfaces** `Editor.svelte:440` `onPlaylistDragOver`/`onPlaylistDrop` (playlist reorder vs `isExternalFileDrag` → `handleExternalFiles`) + `Editor.svelte:1256` `div.external-drop-zone` (`Drop images or videos here`) + `Editor.svelte:1460` `handleTauriPaths` (`SONG_EXTS` vs `ALLOWED_EXTS` `Editor.svelte:40` `MEDIA_FILTERS` → `media.rs:46` `MediaKind`) now share the same `handleExternalFiles` path, so internal drag and OS desktop-file drops both get the fix.

- **Verify by hand:** Drag `photo.jpg` (or `movie.mp4`) onto the playlist (or `external-drop-zone`) → new slide appears in grid with label `photo` (`slideDisplayName` `Editor.svelte:130` `name` = `photo`, `title` = `""`) and `Output` (`SlideRender` `Output.svelte:62` `showText`/`showBackground` `SlideRender.svelte:69`) renders **no text** on the full-bleed `Image`/`Video` background (exactly `Clear text` state). Open detail (`openDetail` `Editor.svelte:243` → `edit-window` `Editor.svelte:1933`) → type `Title` = `Sunset Caption` → `commitTitle` `Editor.svelte:890` → `Output` now shows caption over the same background, `grid-label` updates to `Sunset Caption` if `name` blank else stays `photo` until `Slide name` is edited — manual text still works exactly as before. `npm run check` 0/0, `cargo check` 4 `dead_code`.
...
- **Backend `src-tauri/src/commands.rs:1245` `background_for_kind(app, &SlideKind)` looks up `settings.default_looks.{scripture,song,generic}` → `project.find_look(id)` → `look.background.clone()` if `Some`, else `Background::default()` + `commands.rs:1290` `add_slide(title, body, name, kind: Option<SlideKind>)` `kind_val = kind.unwrap_or(Generic)` `bg = background_for_kind(&app, &kind_val)` `Slide { kind: kind_val, background: bg }` + `commands.rs:415` `add_song_to_playlist` `kind: Song` `bg = default_background_for_kind(Song) or song.default_background` `Slide { kind: Song }` + `commands.rs:1188` `load_template` `kind: it.kind` (TemplateItem now `project.rs:800` `TemplateItem { name, kind, title, body, background }` with `#[serde(default)]` for `kind`) + `commands.rs:1135` `save_template` `kind: s.kind` + `commands.rs:865` `set_default_look(kind: String, look_id: Option<String>)` validates kind `scripture|song|generic` and `look_id` exists, updates `settings.default_looks` + `write_settings` + `log` + `snapshot` + `commands.rs:860` `get_default_looks` + `src-tauri/src/lib.rs:652` handler registration. `LookPatch` `commands.rs:700` `background: Option<Option<Background>>` `#[serde(default)]` + `apply_look_patch` `commands.rs:790` handles `Some(background)` → `look.background = background`.

- **Contract `src/lib/types.ts:51` `SlideKind = "generic"|"song"|"scripture"` + `types.ts:53` `Slide { kind?: SlideKind }` + `types.ts:90` `Look { background?: Background }` + `types.ts:111` `LookPatch { background? }` + `types.ts:185` `DefaultLooks { scripture,song,generic }` + `types.ts:195` `ClientState { defaultLooks }` + `src/lib/sync.ts:117` `addSlide(title?, body?, name?, kind?: string)` + `sync.ts:160` `getDefaultLooks`/`setDefaultLook`.
...
*Extends the Look editor with finer FreeShow-textbox-inspired styling controls — styling only, no drag/resize WYSIWYG canvas (explicitly out of scope). Every control rides the existing optimistic-draft path, so the live preview (`Preview — live sample rendered with this Look, updates instantly`) re-renders on each change exactly like the existing size/font/colour/position controls.*

- **No third element needed:** scripture slides render the reference/translation line as the slide title (`title = reference, body = verse text`, `src-tauri/src/scripture.rs:713`), so per-element **Title + Body** styles fully cover Song/Scripture/Generic slides — the Title section is labelled accordingly (`src/components/LookStyleFields.svelte:1`).
- **Center defaults confirmed (no change needed):** vertical `textPosition` was already `Center` everywhere (`TextPosition::Center` is `#[default]` `src-tauri/src/project.rs:132`, `Look::main_default`/`stage_default` `project.rs:378/398`, both `addLook` creators). Horizontal alignment is new: `HAlign { Left, Center, Right }` with `#[default] Center` (`project.rs:204`), per-element `align: center` in `TextStyle::title_default`/`body_default` (`project.rs:285/295`), so new Looks start centred on both axes.
- **Backend `src-tauri/src/project.rs:218` `TextStyle { align, lineHeight, shadowBlur, shadowX, shadowY, outlineWidth, outlineColor, bgColor, bgOpacity }`** + `project.rs:339/342` `Look { title_style, body_style }` with `#[serde(default = "TextStyle::title_default"/"body_default")]` — legacy `project.json` files deserialize cleanly, and the role defaults reproduce the historic rendering exactly (title `line-height 1.1` + `0 2px 24px` shadow, body `1.4` + `0 2px 20px`). `upsert_look` merges via `commands.rs:712` `TextStylePatch` (all-optional, same philosophy as `LookPatch`) + `commands.rs:843` `apply_text_style_patch` with clamps (line-height `0.8–3.0`, blur `0–60`, offsets `±50`, outline `0–8`, opacity `0–1`); new Looks start from `Look::main_default` so the centred defaults apply even when the create patch omits styles.
- **Contract `src/lib/types.ts:71` `HAlign` + `types.ts:80` `TextStyle`/`TextStylePatch` + `types.ts:96/109` `DEFAULT_TITLE_STYLE`/`DEFAULT_BODY_STYLE`** + `Look { titleStyle, bodyStyle }` (`types.ts:140`) / `LookPatch` (`types.ts:175`).
- **Rendering `src/components/SlideRender.svelte:35`**: per-element `text-align` / `text-shadow` (soft black, historic alphas; `none` when zeroed) / `-webkit-text-stroke` + `paint-order: stroke fill` outline / `rgba()` readability bar with `box-decoration-break: clone` so multi-line bodies get a per-line bar. Line height travels as container vars (`--look-title-line-height`/`--look-body-line-height` `SlideRender.svelte:85/334/344`) rather than inline styles because `fitText`'s measure pass clears inline `lineHeight` — the var survives, keeping auto-fit accurate. Style fields join the `fitText` `deps` token (`SlideRender.svelte:81`) so auto-shrink re-runs on any change. Applied to title, body, and the Stage chord view alike; Output/Stage inherit automatically since both render through `SlideRender`.
- **One shared UI `src/components/LookStyleFields.svelte:1`** (Title + Body sections: align segmented control, line height, shadow blur/X/Y, outline width + colour, bar colour + opacity % with the `black ~50% over video` hint) mounted in **both** editors — central `LookEditorView.svelte:338` (`uid="lev"`) and `Settings → Looks` `SettingsPanel.svelte:1068` (`uid="sp"`, distinct radio-group names) — each via a `setStyle(role, patch)` merge (`LookEditorView.svelte:75`, `SettingsPanel.svelte:107`) into the existing debounced `scheduleCommit` (`LookEditorView.svelte:49`, `SettingsPanel.svelte:123`) and `addLook` defaults (`LookEditorView.svelte:100`, `SettingsPanel.svelte:148`). No drag/resize canvas added — bounding-box editor untouched.
- **Verify:** `npm run check` 0 errors 0 warnings; `cargo check` OK (3 pre-existing `dead_code`: `AudioPlayer::is_active` `audio.rs:390`, `COPY_SUFFIX` `media.rs:16`, `Slide::display_name` `project.rs:117`); `npm run build` bundles clean. `cargo test` does **not** compile on this checkout — pre-existing failure unrelated to this change (`missing field default_looks in Settings literal`, `commands.rs:3113`, reproduces on stashed HEAD). Out of scope, noted: the phone stage-network page (`network.rs:499`) is a separate lightweight renderer and ignores the new style fields (falls back to its own CSS). Manual: open Looks → change Title align/shadow/outline/line-height/bar → preview updates instantly; set bar black 50% over a video-background sample → readable bar; restart → styles persist via autosave (`project.looks`).

---

## Changed (2026-09-06) — Library sidebar readability (swatches, collapse toggle, active-section growth)

*Layout/visual improvement only — no change to library search or drag-and-drop logic. The song list was a cramped text-only list with every song's verses and arrangement chips always expanded inline.*

- **Swatch per song `src/components/Editor.svelte:2132`** — each song row now opens with the same `.swatch` pattern the playlist slide entries use (`Editor.svelte:1913`): solid `defaultBackground` → colour chip, image/video → `background-image: url(thumb)` (`fileUrl`, same helper), live-camera → 🎥 badge. Sizing/cover tweak in `Editor.svelte:3827` `.song-swatch`. Songs are visually distinguishable at a glance, not just text rows.
- **Collapsed/expanded toggle `Editor.svelte:60/2114`** — `expandedSongs: Set<string>` (`$state`, natively reactive) + `toggleSongExpanded`/`isSongExpanded` (`Editor.svelte:61/65`): a 22px chevron (`Editor.svelte:2117` `.song-expand`, `Editor.svelte:3806`) with `aria-expanded` + verse-count tooltip now guards each song's verse list *and* its arrangement Order chips (`{#if songExpanded}` `Editor.svelte:2159`), which were previously always rendered inline. Searching auto-expands every filtered match (`isSongExpanded` returns true while `librarySearch` is non-empty); clearing the search restores the manual toggle state. All verse/song click + drag handlers are byte-identical, only wrapped in the conditional.
- **Active-section growth `Editor.svelte:71/2093/3205`** — `libraryActive` (searching or any song expanded, same `.active`-style convention as the scripture section) adds `library-active` to the section, whose rule (`flex: 2 1 0`) doubles its share against the other `has-content` sidebar sections (`flex: 1 1 0`) instead of leaving the Library permanently squeezed.
- **Verify:** `npm run check` 0 errors 0 warnings; `npm run build` bundles clean. Manual: library with several songs → compact rows (chevron + swatch + title + counts); expand one song → verses + Order chips appear, section grows; type in Search songs → all matches auto-expand; drag song/verse to playlist and click-to-add behave exactly as before.

---

## Changed (2026-09-06) — Optional exit/outro animation on real Quit

*Settings → General → Exit animation picks an optional video (`.mp4` etc.) played full-bleed on the Output and Stage windows when quitting from the tray, so the service ends on video instead of an abrupt cut to desktop. Close-to-tray behavior is unchanged (still just hides the Editor); this only runs on an actual Quit. Layout of the change is backend-driven with dumb renderers, same architecture as everything else.*

- **Setting `src-tauri/src/project.rs:1086` `Settings { exit_animation: Option<String> }`** (default `None` = instant quit, unchanged; legacy `settings.json` loads via struct-level `default`) + `project.rs:667` surfaced on `ClientState` + `commands.rs:986` `set_exit_animation` (trim, empty→`None`, persists, re-emits) + `SettingsField::ExitAnimation` (`commands.rs:2115/2140/2166/2216`) so export/import round-trips and reports it. Frontend: `types.ts:262` `exitAnimation`, `sync.ts:202` `setExitAnimation`, picker + `Clear` + summary row in `SettingsPanel.svelte:275/440/1005`.
- **Quit flow `src-tauri/src/outro.rs:97` `begin_quit`** (tray Quit → `lib.rs:122` `quit_app` sets `QUIT_REQUESTED` then delegates): persists project + library *first* (state safe even if killed mid-outro; `finalize` stays the single writer of `clean_shutdown`), then — only if the file exists and Output and/or Stage is actually showing (fail-open visibility check) — closes the Editor immediately (`lib.rs:207` lets the Editor really close once quit starts; otherwise close still hides to tray), emits `exit-outro {path}`, and spawns a **6s hard-cap thread** (`outro.rs:32/144`) after which `app.exit(0)` runs no matter what. No video / no visible windows / missing file → instant `app.exit(0)`, exactly today's behavior.

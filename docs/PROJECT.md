...
*Audit-first session: every item in the request was verified already implemented (`64b2f4d`, documented in README/PROJECT.md). No code changed; this entry records the verification with `file:line` refs.*

- **Step 1 — video definitively muted:** `<video>` carries a hardcoded `muted` attribute at all times — `Output.svelte:164`/`180`, `SlideRender.svelte:63`/`144` (both live + crossfade frames, Output + Stage + Editor previews). The audio system cannot conflict with video playback; confirmed by attribute reading, no runtime path unmutes.
- **Step 2 — backend `src-tauri/src/audio.rs`:** rodio 0.17 on cpal 0.15 (`Cargo.toml:48-51`, mp3/wav/flac/vorbis decoders); single `AudioPlayer` on its own `std::thread::spawn` worker (`audio.rs:118`) driven by an mpsc `Command` channel (`Load`/`Play`/`Pause`/`Stop`/`SetVolume`/`Seek`/`SetDevice`, `:154-226`); public `load`/`play`/`pause`/`stop`/`set_volume`/`seek`/`set_device` (`:323-375`). Zero `run_on_main`/`get_webview_window` calls in the file — no Windows deadlock surface by construction.
- **Step 3 — routing:** `list_audio_devices` (`commands.rs:1609`, registered `lib.rs:694`) enumerates cpal outputs; `set_audio_device` (`commands.rs:1693`) persists `audio_output_device_id` in Settings (`project.rs:886`, `#[serde(default)]`, same pattern as display selection); selection falls back to system default when unset/unplugged (`audio.rs:133-141`).
- **Step 4 — UI:** Settings → Audio tab (`SettingsPanel.svelte:1306`): device selector + Refresh (`:1320`), Load track (`:1340`), Play/Pause/Stop transport (`:1348`), volume slider with % readout (`:1354`), status line. Not tied to slides — independent utility as scoped. One deliberate gap: `seek_audio` exists in backend + `sync.ts:266` but has no UI control (position/duration aren't live-polled, so a progress slider would be dead); seeking stays a backend capability until position polling is justified.
- **Step 5 — threading:** decode/playback entirely on the player thread; Tauri commands only enqueue `Command`s and snapshot state — main thread never blocks on audio.
- **Verify:** `npm run check` 0/0, `cargo check` clean (3 pre-existing `dead_code`). **Hand-verification NOT possible here** — no audio device in this environment; play/pause/stop/volume through a selected device plus app-responsiveness-during-playback must be confirmed on real hardware (volunteer machine with speakers/headphones).

---

## Changed (2026-09-05) — Live camera / capture-card slide backgrounds (getUserMedia, no new Rust deps)

*Pre-audit confirmed: no camera feature existed (only the unrelated NDI frame-capture seam `broadcast.rs:192` and MIDI "capture" wording). UVC webcams / capture cards presenting as webcams are consumed via the webview's native getUserMedia — zero Rust video processing, zero new dependencies.*

- **Backend, minimal `src-tauri/src/project.rs:45`** — new `Background::LiveCamera { device_id (wire `deviceId` via `rename`), label }` (both `#[serde(default)]`, so legacy `project.json`/`templates.json` deserialize untouched). Stores only which device to use; `device_id` for exact match, `label` as human name + fallback when ids rotate across restarts. `media.rs:84` maps it to `None` in `background_media` (no file → verifier skips, never a broken-image glyph). All other backend paths clone `Background` opaquely — no other arms needed.
- **Contract `src/lib/types.ts:23` (`LiveCameraBackground`) + `:36` (`isLiveCamera`)** — wire `type: "live_camera"`; `isMedia` deliberately still image|video-only (thumb/path consumers type-narrow safely, camera falls through to dark swatch, never a bad `fileUrl`).
- **Stream lifecycle `src/components/CameraFeed.svelte:1`** — exact-`deviceId` → label → default-camera resolution, one default-camera retry on `OverconstrainedError`, `audio: false` always, all tracks stopped on unmount. Inline non-silent states: "Connecting…" + specific messages for denied (`NotAllowedError`), unplugged (`NotFound/Overconstrained`), busy (`NotReadableError`), unsupported window. Full-bleed `object-fit: cover` like recorded video.
- **Muted decision (flagged, not assumed):** camera feed is always muted, consistent with muted recorded-video backgrounds — live camera *audio* is a PA/mixer concern and must never come through the app unexpectedly. A future "camera audio" toggle would be a deliberate separate decision.
- **Resource scoping `SlideRender.svelte:17` (`enableCamera`, default off):** streams open ONLY on Output live + brief fade-overlap leaving frames (`Output.svelte:127`/`141`, on-deck preloader stays file-only per `Output.svelte:46` comment — warming a hidden stream doubles device contention for nothing visible) and the Editor live preview (`Editor.svelte:2495`). Grid thumbs, Stage, Look previews, stage preview render a quiet 🎥 placeholder (`SlideRender.svelte:222`) — devices are never held open by thumbnails.
- **Editor picker `Editor.svelte:1090` (state/fns) + `:2280` (UI):** 🎥 button beside Add-media opens an inline device list (`enumerateDevices`, Refresh); labels hidden pre-permission get an explicit "Enable camera access" gesture (temp stream, immediately stopped); clear errors for denied/no-devices; selection writes `{type:'live_camera', deviceId, label}` via existing `update_slide`. 🎥 badges on the background field (`:2262`) and playlist swatches (`:1810`), same remove flow as media.
- **Verify:** `npm run check` 0/0, `cargo check` clean (3 pre-existing `dead_code`), `cargo test` 53 passed, `vite build` clean. **Hand-verification NOT possible here** — no camera hardware in this environment; on a real machine confirm: permission prompt appears once and grants cleanly, selecting a UVC/capture device shows the live feed full-bleed on Output (muted), advancing away releases the device (camera light off), denied permission shows the inline message (never silent), and a saved camera slide reloads by label if the device id rotated. Tauri webview camera-permission behavior (WebView2/WKWebView grant persistence) is the main unknown — flagged explicitly.

---

## Research (2026-09-05) — True live Output/Stage thumbnails (actual pixels, not re-render)

*Findings-only session, no implementation code. Pre-check confirmed nothing of this form exists: only same-data `SlideRender` previews (`Output.svelte:127` pattern reused in Editor), ffmpeg file thumbnails (`media.rs:151`), and the unwired NDI `send_frame` seam (`broadcast.rs:260`).*

**What desyncs are plausible (motivates the feature):** (a) WebView2 main-thread freeze — our documented history (`PROJECT.md` Windows blocking audit) — JS dead, frame stale; (b) GPU-process crash with JS alive — ack arrives but pixels frozen; (c) wrong display/window placement. A same-data preview catches none of these; pixels catch all three.

**Approach evaluation:**

- **(a) Tauri v2 core screenshot API — does not exist.** No core API; wry PR #1674 (Rust-side `WebView::screenshot()`, visible-region PNG, tested Win11/Linux-Wayland/macOS) is **open/unmerged** — depending on it means a git-patch fork of wry, which collides with Tauri's pinned wry. Dead end until merged upstream; watch item. In-process `CapturePreview`/WebKit-snapshot needs the native webview handle, which wry does not expose to apps.
- **(b1) GDI PrintWindow/BitBlt (and anything built on it, e.g. xcap default, `tauri-plugin-screenshots` which wraps xcap) — disqualified for our windows.** Chromium/WebView2 renders via GPU composition; GDI reads back black/transparent for composited content. Our Output/Stage windows are exactly the case that fails. (Community plugin also adds third-party maintenance risk: ~90 downloads, single maintainer.)
- **(b2) Windows.Graphics.Capture via the `windows-capture` crate — the viable Windows vehicle.** Reads the GPU buffer directly (correct for WebView2, zero window interference, needs Win10 1903+). HWND comes from Tauri's `Window::hwnd()` (stored handle, no main-thread IPC); session + D3D11 live on a dedicated thread in the crate, same isolation pattern as `broadcast.rs`/`midi.rs` — no deadlock surface. New deps: `windows-capture` (target-cfg Windows) + `image` (JPEG encode; pure Rust). xcap's opt-in `wgc` feature is the fallback if `windows-capture` misbehaves.
- **(b3) Linux — defer explicitly.** xcap covers X11 only; native Wayland needs xdg-desktop-portal ScreenCast + PipeWire (consent dialogs, session plumbing, heavy new system deps: `libpipewire`, `libwayland`, clang bindgen). Ubuntu 22.04+ sessions are commonly Wayland, so an X11-only path would silently mislead. Documented deferral, not silent omission.
- **(c) Deferral vs. pragmatic first step — recommend a two-phase plan, Phase 1 now:** an **Output render-ack heartbeat** (~30 lines, zero deps, cross-platform): Output emits `output-applied { liveId, at }` after each state apply plus a slow heartbeat; Editor shows "Output confirmed slide X · Ns ago" vs a stale warning. Catches case (a) — the historically real failure — immediately. **Phase 2 (Windows-only):** WGC thumbnails at ~1–2s cadence, downscaled JPEG over an event, shown beside the existing preview as "Actual screen"; Linux deferred per (b3). Phase 2 is a real project (new module + thread + encode + UI), not a quick win — hence no code this session, plan described first per instructions.

---

## Changed (2026-09-05) — Phase 1 render-ack heartbeat (Output/Stage proof-of-life)

*Implements the research Phase 1 as specified: one-way acks + slow heartbeat, backend receipt-stamping, Editor confirmed/stale indicators. Zero new deps, no snapshot churn, no window calls on any ack path.*

- **Contract `project.rs:422` (`RenderAck`/`AckUpdate`, camelCase):** `at` stamped by the backend (sender clock never trusted), `liveId` informational. **State `state.rs:56` (`output_ack`/`stage_ack` `RwLock`s) + `:131` (`record_ack`):** two short lock writes, returns both windows' latest — never saves, never snapshots, never touches windows.
- **Listeners `lib.rs:406`:** one-way `output-ack`/`stage-ack` frontend `emit`s (Tauri global listeners in setup) → `record_ack` → tiny `ack-update` fan-out event. No command, no response, no `run_on_main` anywhere near — the Windows deadlock fixes are unaffected by construction. Unit-tested (`state.rs:143`, independent per-window tracking + fail-open to output).
- **Renderers:** `Output.svelte:13`/`96` (`ACK_MS = 5000`) — ack on every applied state + initial mount + 5s interval, fire-and-forget with cleanup; `Stage.svelte:10`/`40` — same on its `current` slide. A frozen renderer simply stops acking (covers frozen-during-change AND frozen-while-idle).
- **Transport `sync.ts:55` (`emitRenderAck`, emit + swallowed errors for browser preview) + `:64` (`subscribeAck`).**
- **Editor `Editor.svelte:203` (ack state, `ACK_STALE_MS = 12000`, 1s aging ticker `:1776`):** per-window lines under each preview — subtle `✓ Confirmed Ns ago` (`:2550` Output, `:2638` Stage), dim idle before first ack, warning-amber `⚠ No confirmation for Ns — Output/Stage may be frozen` (`role="alert"`, `:2553`/`:2641`, CSS `:3283`). Shown only while the window exists (`output.visible`/`stage.visible` gates) — no phantom warnings for uncreated windows.
- **Verify:** `npm run check` 0/0, `cargo check` clean (3 pre-existing `dead_code`), `cargo test` 54 passed (incl. new ack test), `vite build` implied clean via check. **Live behavior NOT verifiable here** — no display server (no xvfb) so the app cannot run: neither the passive "indicator keeps updating" check nor a stall simulation was feasible. Needs a real run: normal use should show `✓ Confirmed Ns ago` cycling ≤5s on both panels; killing/freezing the Output window should flip its line to the stale warning within ~12s while Stage stays fresh.

---

## Research (2026-09-05) — NDI receive / frame-pull from a network source

*Findings-only session, no implementation code. Scoped as the likely heaviest remaining Tier 3 item; stops here for a joint build/defer decision.*

**Pre-audit (explicit): no NDI-receive capability exists.** `src-tauri/src/broadcast.rs:147` `load_ndi` resolves exactly 5 symbols (`NDIlib_initialize`, `NDIlib_destroy`, `NDIlib_send_create`, `NDIlib_send_destroy`, `NDIlib_send_send_video_v2` — `broadcast.rs:157-169`) — send-only. No `NDIlib_find_*` or `NDIlib_recv_*` symbol, struct, or command exists anywhere in the tree (the only "capture" hits are the unwired send-side `send_frame` seam `broadcast.rs:260` and MIDI-trigger "capture" wording). The documented "send-only" scope (`PROJECT.md:133`, `README.md:232`) is accurate.

**1. What the receive API actually requires (separate from send).** Same DLL (`Processing.NDI.Lib.x64.dll` / `libndi.so.5`, `broadcast.rs:127`), same `NDIlib_initialize` — but two new API families on top of it (~12+ entry points vs send's 5):
- **Discovery (`NDI-FIND`):** `NDIlib_find_create_v2` → `NDIlib_find_wait_for_sources` (blocking wait-with-timeout until the set changes) → `NDIlib_find_get_current_sources` (returns `NDIlib_source_t { p_ndi_name, p_url_address }` list; memory owned by the finder, valid only until next call) → `NDIlib_find_destroy`. Discovery is self-contained P2P mDNS (no Bonjour needed on Windows; on Linux the **avahi-daemon must be running** — new runtime note, Windows-first means this mostly bites dev/CI). It takes **seconds** to converge; early listings are incomplete, so the UI needs a "scanning…" state, not a one-shot dropdown. Supports groups + `p_extra_ips` for cross-subnet sources (church VLANs take note).
- **Receive (`NDI-RECV`):** `NDIlib_recv_create_v3` (with `NDIlib_recv_create_v3_t`: source + `color_format` + `bandwidth` + `allow_video_fields` + name) → `NDIlib_recv_connect` (re-point/disconnect any time; a receiver **stays "connected" and auto-reconnects** if the source vanishes) → `NDIlib_recv_capture_v2/v3` (blocking call with timeout-ms, returns a frame-type enum: video / audio / metadata / none / status_change) → **`NDIlib_recv_free_video_v2` (mandatory — new buffer-ownership discipline; send is fire-and-forget, receive must free every frame)** → `NDIlib_recv_destroy`. Plus optional tally/metadata (`NDIlib_recv_set_tally`, connection metadata to request a preferred format from the source).
- **Licensing:** no new burden. Basic find+receive is in the **standard royalty-free SDK** — same install, same terms as the existing send integration. Only the Advanced SDK extras (compressed pass-through `color_format_compressed_*`, HW-accelerated decode, NDI 6.2 Discovery-Server receiver advertisement, recording) are royalty-based/vendor-ID. We would not need any of them: requesting `NDIlib_recv_color_format_BGRX_BGRA` makes the SDK hand us **BGRA/BGRX directly** (zero conversion code, matches the existing send-side pixel format), while `fastest`/`best` modes return UYVY/UYVA/P216/PA16 and would force a new YUV→RGB conversion path — so the create-struct choice is load-bearing and the answer is "ask for BGRA".
- **Frame struct:** `NDIlib_video_frame_v2_t` is the *same struct already mirrored* in `broadcast.rs:76` — but on receive, resolution/framerate/stride are **dynamic per frame** (stride may ≠ width×4; must copy out of the SDK-owned `p_data` then free). Sources: official NDI SDK docs (`docs.ndi.video` NDI-RECV / NDI-FIND / Frame Types pages) + SDK header `Processing.NDI.Recv.h` (mirrored in DistroAV repo) + SDK `NDIlib_Recv`/`NDIlib_Find` C++ examples.

**2. Getting a received frame into the Svelte frontend.**
- **(a) Base64 snapshots (confidence monitor): realistic and small.** Recv thread → downscale (e.g. 480–640px wide) → JPEG-encode (new dep: pure-Rust `image` crate, ~50–150 ms per 1080p encode on modest hardware — hence the downscale-first + low rate) → base64 (+33% size) → dedicated Tauri event (e.g. `ndi-preview`) → `<img src="data:image/jpeg;base64,…">`. Realistic: **1–3 fps at thumbnail size, 0.5–2 s latency**. Fine for "is the camera pointed at the right thing" — not live video. Complexity: days, one new module reusing the `broadcast.rs` thread pattern + `image` dep. Hard constraints: must be a **dedicated event, never the `state` broadcast** (`commands.rs:98` `snapshot_and_emit` fans out to every window on every mutation — stuffing MB-scale strings through it would punish all three windows); JPEG quality/size caps to keep each event ~20–60 KB.
- **(b) Local WebRTC bridge (real video): a new subsystem, not an extension.** Needs an H.264/VP8 encoder (ffmpeg CLI pipe or a native encoder crate) + a WebRTC stack (`webrtc-rs` — enormous dep tree — or a local WHIP server) + signaling + ICE (even loopback) + peer-connection lifecycle + an Svelte `RTCPeerConnection` client. Realistic: **15–30 fps, <300 ms loopback latency** — but weeks of work, new failure modes (ICE, codec negotiation), and a dependency footprint wildly out of proportion to the rest of the backend (which deliberately uses CLI ffmpeg, not media frameworks). Complexity is comparable to a whole new Tier, as suspected.
- **(c) Third option found during research — MJPEG over localhost HTTP (the sweet spot if (a) feels too choppy):** serve `multipart/x-mixed-replace` on 127.0.0.1 (a tiny HTTP responder on its own thread/port; `network.rs:126` already proves the "socket on a spawned thread" pattern) and point a plain `<img src="http://127.0.0.1:PORT/ndi.mjpeg">` at it. No base64 bloat, the browser handles framing, JPEG-encode-only (same `image` dep as (a)). Realistic: **~10–15 fps at 640px on localhost**, a weekend-scale build — no signaling, no ICE, no encoder negotiation. Loses to WebRTC on latency/efficiency, wins overwhelmingly on simplicity. **Recommendation between the three: (a) now if any in-app preview is wanted, (c) as the designed next step, (b) never unless a hard sub-300 ms requirement appears.**

**3. Complexity vs send — assumption CONFIRMED, roughly 3–4× surface.** Send = 5 symbols, fixed 1080p29.97 output (`broadcast.rs:45`), no discovery, no buffer ownership, no per-frame variance, and no frontend-delivery problem (bytes go to the LAN, done). Receive = 3 API families (find + recv + free/tally), async discovery UX, must-free ownership, dynamic formats, a colorspace decision, a new `image` dependency, a new thread with two blocking loops (find-wait + capture), plus the entire pixels→webview delivery design above. The `libloading` runtime-load pattern transfers directly, but that is the only cheap part.

**4. Windows main-thread / GPU risk classes that apply here.**
- **Blocking SDK calls must stay on dedicated threads (same rule as ever):** `NDIlib_find_wait_for_sources` and `NDIlib_recv_capture_v3` both block with timeouts up to seconds — calling either from a `#[tauri::command]` handler would stall that invoke (and risk pool starvation); calling from the main thread would replay the WebView2 freeze saga (`PROJECT.md` Windows Blocking Audit). The `broadcast.rs`/`osc.rs` dedicated-thread + timeout-loop + flag/join shutdown pattern covers both, but this needs **two** blocking loops (finder + receiver) with clean teardown — flag it in design, not as a blocker.
- **Frontend delivery is the new freeze-adjacent surface:** pushing frames through the `state` event / `snapshot()` path would serialize MB-scale base64 through WebView2 IPC to all windows per frame — same *class* of bug (message-loop saturation) as the 2026-08-31/09-01 freezes, even though the mechanism differs. Dedicated throttled event + small JPEGs avoids it by construction. A `<video>`/`<img>` element itself is the browser's own compositor path (same as existing muted `<video>` backgrounds) — not the broken pattern; the broken pattern was ever main-thread-blocking IPC.
- **Stale-frame-mistaken-for-live (new trust hazard, no send-side equivalent):** the receiver auto-reconnects and holds connection across source loss, so without explicit `none`-timeout / `status_change` handling the UI can show a frozen last frame that *looks* live. Any preview must carry a fresh/stale indicator (same UX lesson as the 2026-09-05 render-ack heartbeat: proof-of-life beats pixels alone).
- **Linux/avahi + church-network notes:** discovery needs avahi-daemon on Linux (document, Windows-first means low pain); cross-VLAN sources need `p_extra_ips`/Discovery Server config, not code — an operator-docs line, not a feature.

**Recommendation: SCOPE DOWN, do not build full-rate now.** Approve at most the (a) confidence-monitor snapshot path (2 fps JPEG thumbnails + source picker + stale indicator) as a bounded Tier-3-lite item if camera-input workflows demand an in-app preview; design its event channel so (c) MJPEG can replace the transport later without touching the receiver. Defer (b) WebRTC explicitly and indefinitely. If no near-term service flow needs seeing the network camera inside the app (external NDI Studio Monitor covers the "is it up" case for free), deferring even (a) is defensible — this remains the heaviest Tier 3 item and nothing in these findings makes it lighter, only better-bounded.

---

## Changed (2026-09-05) — NDI receive confidence monitor (~2 fps preview + picker + stale)

*Implements the scoped-down research option (a): a low-rate "is this camera alive and pointed at the right thing" check. Explicitly NOT live video. Fully independent from NDI send — separate toggle, thread, SDK handles.*

- **Backend `src-tauri/src/ndi_receive.rs:1` (new, 921 lines)** — same `libloading` runtime-load pattern as `broadcast.rs` (missing SDK = clear error, app keeps working). FFI values verified against SDK headers, not recalled: `COLOR_BGRX_BGRA = 0` + `BANDWIDTH_HIGHEST = 100` (`RecvCreateV3` `ndi_receive.rs:119`), frame types none/video/error/status_change = 0/1/4/100, `FindCreate`/`NdiSourceRaw` layouts (struct-size unit tests `ndi_receive.rs:823`).
- **Discovery** `ndi_receive.rs:579` `monitor_thread` — one dedicated `ndi-monitor` thread owns lib + finder + receiver; `find_wait_for_sources(500ms)` ticks (never a hot spin) refresh the shared list + emit dedicated `ndi-sources` events. `list_ndi_sources` `commands.rs:945` returns the snapshot (empty while mDNS converges — the UI shows "scanning…", never a one-shot dropdown).
- **Receive** — `recv_create_v3` requests BGRX_BGRA explicitly (`allow_video_fields: 0` → always progressive), so frames arrive BGRX/BGRA and **no YUV path exists**. `recv_capture_v2` (`commands.rs` never sees it) runs with video-only + 250 ms timeout; every captured frame is copied then **freed immediately** (`recv_free_video_v2`, even on validation failure) before any processing. `Control::Connect/Disconnect` `ndi_receive.rs:400` switches sources (`recv_connect(NULL)` + `recv_destroy`, no leaks); `stop()` `ndi_receive.rs:522` joins the thread (bounded by the 250 ms timeout) then destroys receiver → finder → SDK.
- **Delivery seam** `PreviewFrame` `ndi_receive.rs:293` + `deliver_preview_frame` `ndi_receive.rs:306` — capture produces small RGB; only that one function knows the transport (today: 560 px downscale `PREVIEW_WIDTH` `ndi_receive.rs:72` nearest-neighbor BGR→RGB, JPEG q60 via `image`, base64, dedicated `ndi-preview-frame` event throttled to ~2 fps `EMIT_PERIOD`). **Never `snapshot_and_emit`** — the `state` broadcast is untouched by construction. MJPEG-over-localhost later = replace that one function.
- **Stale safety** — `is_stale` (4 s `STALE_AFTER`) watchdog on every tick + immediate STALE on `FRAME_ERROR`: `Live` only while frames arrive; `Stale` the moment the feed quiets (auto-reconnect can otherwise hold a frozen last frame that looks live). Status rides the dedicated `ndi-monitor-status` event (`NdiMonitorState` `off/scanning/connecting/live/stale/error`).
- **Wiring** — `NdiReceiveMonitor` on `AppState` (`state.rs:32`, default `:75`); 6 commands `commands.rs:929` (`start/stop/list/status/connect/disconnect`, all returning small types, zero snapshots); registered `lib.rs:672`; torn down in `finalize` `lib.rs:73`. New deps `Cargo.toml` (`image` JPEG-only pure-Rust + `base64`). Send path untouched (verified: no `broadcast.rs`/`commands.rs` send-code changes).
- **UI** `SettingsPanel.svelte:839` "NDI camera monitor (preview ~2 fps)" block beside the broadcast toggle: enable → scan, source buttons (selected highlighted), `data:image/jpeg;base64` preview with LIVE green-glow vs STALE amber border + "STALE — last frame, not live" veil (`SettingsPanel.svelte:252` state, `:856` template, semantic tokens only — no new colors). Previous source's frame is dropped on (re)connect; closing Settings stops the monitor (`onDestroy`) so no camera connection is held without a viewer. Contract `types.ts` (`NdiSourceInfo`/`NdiMonitorStatus`/`NdiPreviewFrame`) + `sync.ts:76` (3 subscribes + 6 invokes).
- **Verify:** `cargo check` clean (3 pre-existing `dead_code` only), `cargo test` 62 passed (8 new: downscale/swizzle/scale/stale/contract/FFI-constants), `npm run check` 0/0. **Live round-trip verified in-env** (temporary test, removed after — suite stays SDK-independent): this machine unexpectedly HAS `libndi.so.5` + running avahi-daemon, so a local test sender was discovered via mDNS, connected with the production struct, captured 320×180 BGRA (`fourcc 0x41524742`), downscaled real bytes correctly, tore down clean — all in 0.92 s. Env quirks noted: bare-soname dlopen needed `LD_LIBRARY_PATH=/usr/local/lib` here, and NDI codec round-trips are not bit-exact (±few LSBs — tolerated, never asserted in shipped code). **Still needs your hands:** real camera source selection, SDK-missing messaging on Windows, multi-hour monitor stability.

---

## Changed (2026-09-02) — Fix NDI black frame + Output self-healing (Editor close)

*Production logs: closing Editor destroyed Output and self-healing failed with `Output window not pre-created`, while OBS discovered `MakrStudio - Sunday Output` but showed black. Root-caused as window lifecycle + unwired frame source.*

- **Root-cause `lib.rs:601` + `windows.rs:531`/`293`/`1026`** Deferred `precreate_hidden_windows` 180ms via `spawn+sleep+run_on_main_thread` left a race where `get_webview_window(OUTPUT_WINDOW)` was `None` when Editor was hidden quickly; `move_output_to`/`ensure_output` Windows fallbacks only scheduled a deferred build via `run_on_main_thread` fire-and-forget and returned `Err("not pre-created — deferred build scheduled")`, which the `Destroyed` handler `windows.rs:376` logged as `FAILED` without retry. Editor `CloseRequested` (`lib.rs:197` hide) and Output were not independent in that window.

- **Fix `lib.rs:601` synchronous precreate + `windows.rs:1026`/`293` blocking rebuild** `lib.rs:601` now calls `precreate_hidden_windows` synchronously on setup main thread (no 180ms), so Output exists before user can close Editor. `move_output_to` Windows fallback now `run_on_main(&app_main, &*app_clone.state(), "move_output_to_create_output", move || builder.build())` and attaches `on_window_event Destroyed` handler, returning `Ok(window)`; `ensure_output` fallback similarly builds via `run_on_main` with `catch_unwind` and handler. Both now **complete** and log `self-healing complete` not `FAILED`. Editor close now only hides Editor (`prevent_close`+`hide`), Output stays alive as designed (`prevent_exit` unless `QUIT_REQUESTED` `lib.rs:743`).

- **NDI frame source `broadcast.rs:25`/`347` `send_frame` seam** Confirmed: pixel data comes from Output WebView via future offscreen capture mirroring `SlideRender` (currently unwired, `current: None`, so OBS showed black despite source discovery via `NDIlib_send_create`). `spawn_send_thread` holds last-good-frame and re-sends every 33ms; if no frame ever, it sent nothing (black).
...
## Changed (2026-09-02) — Fix media slide Title vs name (only name from filename)

*Media-background slide creation (drag-and-drop or `Add media`) previously set the filename as the on-screen `Title` (`Editor.svelte:864` `addSlide(base, "")`), requiring manual clear for a clean full-bleed background. Now only `name` (grid/playlist label) comes from the filename — `Title`/`Body` stay empty, so `Image`/`Video`/`LiveCamera` (not `Solid`) slides default to no visible on-screen text overlay, but the grid still shows a sensible name. Operator can still add text via `Title`/`Body` fields.*
